  ...
  useEffect(() => {
    ...
    const onMove = (e: MouseEvent) => {
      ...
      const idx = Math.max(
        ...
        Math.min(
          data.length - 1,
          Math.round((svgX / SVG_W) * (data.length - 1)),
        ),
      );
      ...
      if (ln) {
        ln.setAttribute("x1", dot.x.toFixed(1));
        ln.setAttribute("x2", dot.x.toFixed(1));
        ln.style.display = VISIBLE;
      }
      ...
      if (tm) {
        tm.setAttribute("x", (ttX + 12).toFixed(1));
        tm.setAttribute("y", (ttY + 20).toFixed(1));
        tm.textContent = d.time;
        tm.style.display = VISIBLE;
      }

      // separator
      const sep = ttSepRef.current;
      if (sep) {
        sep.setAttribute("x1", (ttX + 12).toFixed(1));
        sep.setAttribute("x2", (ttX + 108).toFixed(1));
        sep.setAttribute("y1", (ttY + 26).toFixed(1));
        sep.setAttribute("y2", (ttY + 26).toFixed(1));
        sep.style.display = VISIBLE;
      }

      // temp dot + text
      const td = ttTDotRef.current;
      if (td) {
        td.setAttribute("cx", (ttX + 18).toFixed(1));
        td.setAttribute("cy", (ttY + 40).toFixed(1));
        td.style.display = VISIBLE;
      }
      const tt = ttTempRef.current;
      if (tt) {
        tt.setAttribute("x", (ttX + 28).toFixed(1));
        tt.setAttribute("y", (ttY + 44).toFixed(1));
        tt.textContent = `${d.temp}°C`;
      ...
      if (hasPrecip) {
        ...
        if (pd) {
          pd.setAttribute("cx", (ttX + 18).toFixed(1));
          pd.setAttribute("cy", (ttY + 57).toFixed(1));
          pd.style.display = VISIBLE;
        }
        const pt = ttPrecipRef.current;
        if (pt) {
          pt.setAttribute("x", (ttX + 28).toFixed(1));
          pt.setAttribute("y", (ttY + 61).toFixed(1));
          pt.textContent = `${d.precip}% ${t("weatherConditionRain")}`;
          pt.style.display = VISIBLE;
        }
      } else {
        hide(ttPDotRef.current);
        hide(ttPrecipRef.current);
